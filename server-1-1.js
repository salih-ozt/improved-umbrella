...
app.post('/api/disease-alarms', authenticateToken, async (req, res) => {
    try {
        ...
        if (!city || !cropType || !diseaseInfo) {
            return res.status(400).json({ error: 'city, cropType ve diseaseInfo zorunludur.' });
        }
        ...
        await pool.query(
            ...
            [alarmId, req.user.id, city.trim(), (district||'').trim(), cropType.trim(),
             diseaseInfo.substring(0,1000), (note||'').substring(0,500)]
        );

        // Aynı il + ürün türünde çiftçileri bul
        // farmerCrops alanında cropType geçen kullanıcılar VEYA location/farmerCity eşleşenleri
        const targets = await pool.query(
            `SELECT id FROM users
             WHERE id != $1
               ...
               AND "isBanned" = FALSE
               AND (
                   (LOWER("farmerCity") = LOWER($2) OR LOWER(location) LIKE LOWER($3))
                   AND ("farmerCrops" ILIKE $4 OR "farmerCrops" IS NULL OR "farmerCrops" = '')
               )
             LIMIT 500`,
            [req.user.id, city.trim(), `%${city.trim()}%`, `%${cropType.trim()}%`]
        );
